...
import { DbWrapper, Block, Tx } from "../srcs/db-wrapper";
...
describe('DbWrapper', () => {
    ...
    before(async () => {
        ...
        await db.insertBlocks(blockData.map(db.toBlock));
        await db.insertTxs(txData.map(db.toTx));
    })

    it('findBlockByBlockHash test', async () => {
        let blockHash = "0xe1aa864df15b313851446b97ac4ff3b7fbd480f5ce0bbc316a3931ad3b51834b";
        let block: Block = (await db.findBlockByBlockHash(blockHash))!;

        expect(block.blockHash).to.be.equal(blockHash);
    });


    it('findBlockByBlockHash fail test', async () => {
        let blockHash = "0xe1aa864xxxxxxxxxxxxxb97ac4ff3b7fbd480f5ce0bbc316a3931ad3b51834b";
        let block: Block | null = await db.findBlockByBlockHash(blockHash);

        expect(block).to.be.null;
    });

    it('findBlockByHeight test', async () => {
        let height: number = 8780938;
        let block: Block  = (await db.findBlockByHeight(height))!;

        expect(block.height).to.be.equal(height);
    });

    it('findBlockByHeight fail test', async () => {
        let height: number = 8780900;
        let block: Block | null = await db.findBlockByHeight(height);

        expect(block).to.be.null;
    });

    it('findLatestBlocks test', async () => {
        let blocks: Block[] = await db.findLatestBlocksSortByHeight(2);

        expect(blocks.length).to.be.equal(2);
        expect(blocks[0].height).to.be.equal(8780939);
        expect(blocks[1].height).to.be.equal(8780938);
    });

    it('findBlocksSortByHeight test', async () => {
        let blocks: Block[] = await db.findBlocksSortByHeight(8780939, 2);

        expect(blocks.length).to.be.equal(2);
        expect(blocks[0].height).to.be.equal(8780939);
        expect(blocks[1].height).to.be.equal(8780938);

        blocks = await db.findBlocksSortByHeight(8780938, 2);

        expect(blocks.length).to.be.equal(1);
        expect(blocks[0].height).to.be.equal(8780938);
    });

    it('findBlocksSortByHeight fail test', async () => {
        let blocks: Block[] = await db.findBlocksSortByHeight(8780930, 2);

        expect(blocks.length).to.be.equal(0);
    });


    it('findTxByTxHash test', async () => {
        let txHash = '0x5feb597f641a6f1d30f719993b762d8e4f8eef41f9c7a9f22b0939cf72c537cc';
        let tx: Tx = (await db.findTxByTxHash(txHash))!;

        expect(tx.txHash).to.be.equal(txHash);
    });

    it('findBlockByTxHash fail test', async () => {
        let txHash = '0x5feb59xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx9c7a9f22b0939cf72c537cc';
        let tx: Tx | null = await db.findTxByTxHash(txHash);

        expect(tx).to.be.null;
    });

    it('findTxsByAccountSortByBlockNumber test', async () => {
        let account = '0x5f7efa06f6380885697683c874db4123f7a3827d';
        let txs: Tx[] = await db.findTxsByAccountSortByBlockNumber(account, 10);
